...
function Install-SccmMissingUpdates
{
  ...
  Process
  {
    ...
    $pendingreboot = Get-PendingReboot $ComputerName
    If ($pendingreboot.RebootPending -eq $true) 
    {
        Write-Verbose "$ComputerName has restart pending, restarting...."
        Restart-Computer -ComputerName $ComputerName -Force -Wait -For WinRM
        while ((get-service -ComputerName $ComputerName -name ccmexec).status -ne 'running')
        {
            Write-verbose 'Waiting for ccmexec, sleep 30s'
            Start-Sleep -Seconds 30 
        }
    }
    
    #Trigger SCCM Update Scan and wait a little
    Write-Verbose 'Trigger updatescan, sleep 60s'
    Invoke-WmiMethod -NameSpace root\ccm -Class sms_client -Name TriggerSchedule '{00000000-0000-0000-0000-000000000113}' -ComputerName $ComputerName | out-null
    ...
    If ($pendingreboot.RebootPending -eq $true) 
    {
      Write-Verbose 'Reboot pending'    
      Restart-Computer -ComputerName $ComputerName -Force -Wait -For WinRM
      $speak.Speak("$ComputerName has finished restarting")
      $speak.Dispose()
    ...
    Else
    {
      Write-Verbose 'Reboot not needed'
    }
        
    Write-Verbose "Patching Done for $ComputerName"
  }
